  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:px-4 animate-fade-in ">
      <div className="lg:col-span-8 space-y-10">
        ...
        <section className="glass rounded-3xl md:rounded-[2rem] shadow-sm p-4 md:p-8 transition-all hover:shadow-md animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-8 pb-3 md:pb-4 border-b border-slate-200/50">
            <div className="p-2 bg-brand-100/50 rounded-lg md:rounded-xl">
              <CheckCircle2 className="w-5 h-5 md:w-6 md:h-6 text-brand-600" />
            </div>
            <h2 className="text-xl md:text-2xl tracking-tight text-slate-900 font-bold">
              Shipping To
            </h2>
          </div>
          <div className="bg-slate-50/50 p-4 md:p-6 rounded-xl md:rounded-2xl border-2 border-slate-100">
            <p className="font-bold text-slate-800 text-sm md:text-lg tracking-tight mb-1 md:mb-2">{shippingAddress.fullName}</p>
            <div className="text-[10px] md:text-sm text-slate-500 space-y-1 font-medium leading-relaxed">
        ...
        <section className="glass rounded-3xl md:rounded-[2rem] shadow-sm p-4 md:p-8 transition-all hover:shadow-md animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-8 pb-3 md:pb-4 border-b border-slate-200/50">
            <div className="p-2 bg-slate-100 rounded-lg md:rounded-xl">
              <ShieldCheck className="w-5 h-5 md:w-6 md:h-6 text-slate-700" />
            </div>
            <h2 className="text-xl md:text-2xl tracking-tight font-bold text-slate-900">
              Payment Method
            </h2>
          </div>
          ...
          <div className="space-y-3 md:space-y-4">
            {paymentMethods.map((method) => (
              <div 
                ...
                className={`group p-3 md:p-5 border-2 rounded-xl md:rounded-2xl cursor-pointer transition-all duration-300 flex items-center justify-between ${
                  selectedMethod === method.id 
                    ? "border-brand-500 bg-brand-50/50 shadow-md ring-2 md:ring-4 ring-brand-50/50 transform scale-[1.02]" 
                    : "border-slate-200/60 hover:border-brand-300 bg-white/60 hover:bg-white/90 hover:shadow-sm"
                }`}
              >
                <div className="flex items-center gap-3 md:gap-5">
                  <div className={`w-10 h-10 md:w-14 md:h-14 rounded-lg md:rounded-xl flex items-center justify-center transition-colors shrink-0 ${selectedMethod === method.id ? 'bg-white shadow-sm' : 'bg-slate-100 group-hover:bg-brand-50'}`}>
                    <method.icon className={`w-4 h-4 md:w-6 md:h-6 transition-colors ${selectedMethod === method.id ? "text-brand-600" : "text-slate-500 group-hover:text-brand-500"}`} />
                  </div>
                  <div>
                    <p className={`font-bold text-sm md:text-lg tracking-tight transition-colors ${selectedMethod === method.id ? "text-brand-900" : "text-slate-800"}`}>
                      {method.name}
                    </p>
                    <p className="text-[10px] md:text-sm text-slate-500 font-medium">{method.description}</p>
                  </div>
                </div>
                <div className={`w-5 h-5 md:w-6 md:h-6 rounded-full border-2 flex items-center justify-center transition-all shrink-0 ${
                  ...
                  {selectedMethod === method.id && (
                    <div className="w-2.5 h-2.5 md:w-3 md:h-3 rounded-full bg-brand-600 animate-in zoom-in duration-200"></div>
                  )}
                </div>
              </div>
            ))}
